...
class CheckInOut(Base):
    ...
    def __repr__(self):
        return f"id: {self.id}, roomId: {self.roomId}, checkin: {self.checkin}, checkout: {self.checkout}"
...
def api_checkin(userId):
    if request.method == "POST":
        ...
        try:
            ...
            print(e)
            data = {"message": str(e), "action": "/checkin"}
            return jsonify(data), 409
    else:
        chk = session.query(CheckInOut).filter_by(userId=userId, checkout=None).first()
        if chk:
            data = {"checkin": chk.roomId}
            print(data)
...
def api_checkout():
    if request.method == "POST":
        ...
        try:
            ...
            print(e)
            data = {"message": str(e), "action": "/checkout"}
            return jsonify(data), 409

@app.route("/API/<path:roomId>/users")
def usersCheckedin(roomId):
    checks = session.query(CheckInOut).filter_by(roomId=roomId, checkout=None).all()
    usernames = [chk.userId for chk in checks]
