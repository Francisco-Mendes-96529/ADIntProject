...
@app.route("/API/restaurant/<resId>", methods=["GET"])
def getMenuEndpoint(resId):
    if request.method == "GET":
        menu = getMenu(resId)
        food_list = [item["Food"] for item in menu]
        resName = session.query(Restaurant).filter_by(id=resId).first().name
        return jsonify({"food": food_list, "name": resName, "type": "menu"})
...
def evaulation():
    if request.method == "POST":
        ...
        restaurantId = request.json.get("resId")
        evaluation = request.json.get("eval")
        rating = request.json.get("rating")

        try:
            newRating(userId, restaurantId, evaluation, rating)
        except Exception as e:
            # If an error occurs in newRating
            return e, 500

        return "Evaluation Successfull", 201
